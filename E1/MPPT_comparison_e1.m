...
for j=1:iter_max
 ...
 for i=1:step_max
    ...
    count=count+1;  
    [Vn_Pyo,Pn_Pyo,delta_P_Pyo,delta_V_Pyo] = PyO(Po_Pyo,Vo_Pyo,delta_P_Pyo,delta_V_Pyo,action,Go,To);
    Vo_Pyo=Vn_Pyo;
    Po_Pyo=Pn_Pyo;
    
    [Vn_IC,Pn_IC,In_IC,Pend,delta_V_IC,delta_I_IC] = IC_control(Vo_IC,Io_IC,Po_IC,Pend,delta_V_IC,delta_I_IC,action,Go,To);
    Vo_IC=Vn_IC;
    Po_IC=Pn_IC;
    Io_IC=In_IC;

     [Vn_QL,Pn_QL]=test_QL_e1(Vo_QL,Po_QL,state_list,actions,Q_table,Go,To);
     Vo_QL = Vn_QL;
     Po_QL = Pn_QL;
...
Vr_n=int16(Vo_S*10);
index_isv = find(state_list(:,1)==Vr_n);
selected_states_v = [index_isv, state_list(index_isv,2)];
