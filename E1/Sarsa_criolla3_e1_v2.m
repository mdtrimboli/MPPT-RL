...
[Po,Vo,Io]=altpvmodel(Go,To,Vr); %(G,T,V)
 ...
 % Search initial state
    Vr_n=int16(Vo*10);
    index_isv = find(states(:,1)==Vr_n);
    selected_states_v = [index_isv, states(index_isv,2)];
...
% Steps loop
    ...
    %Define next state index
        Vf_m=int16(Vf*10);
        index_isv = find(states(:,1)==Vf_m); %List of indexes of the possible next states
        selected_states_v = [index_isv, states(index_isv,2)]; % [Index selected for Vf, Possibles Pf]
